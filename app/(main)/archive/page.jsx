...
export default function MyArchiveView({ onBack }) {
  ...
  const myUploads = [
    { id: "UP-902", title: "Data Structures - End Sem 2023", dept: "CSE", status: "Indexed", date: "2024.03.15", size: "2.4 MB", type: "PDF" },
    { id: "UP-884", title: "Algorithm Analysis Notes", dept: "CSE", status: "Indexed", date: "2024.03.10", size: "1.1 MB", type: "DOCX" },
    { id: "UP-741", title: "Operating Systems - PYQ", dept: "CSE", status: "Scanning", date: "2024.02.28", size: "1.9 MB", type: "PDF" },
  ];
  ...
  return (
    <div className="h-max-screen w-full flex flex-col bg-[#F8FAFC] dark:bg-[#030303] transition-colors duration-500 font-sans selection:bg-blue-500/30">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-blue-600/80">Secured_Vault_v4</span>
              </div>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto w-full px-4 md:px-8 py-6 flex flex-col gap-6">
        ...
        {/* 2. STATS GRID (SCROLLABLE ON MOBILE) */}
        <div className="flex md:grid md:grid-cols-4 gap-3 overflow-x-auto pb-2 md:pb-0 no-scrollbar snap-x">
          {[
            { label: "Rank", val: `#${userStats.rank}`, icon: Zap, color: "text-blue-500" },
            { label: "Streams", val: userStats.uploads, icon: CloudUpload, color: "text-purple-500" },
            { label: "Credits", val: userStats.points, icon: ShieldCheck, color: "text-emerald-500" },
            { label: "Integrity", val: `${userStats.reputation}%`, icon: Activity, color: "text-orange-500" },
          ].map((stat, i) => (
            <div key={i} className="min-w-[140px] flex-1 bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/10 p-4 rounded-2xl snap-center">
              <p className="text-[8px] font-black uppercase tracking-widest text-zinc-400 mb-1">{stat.label}</p>
              <div className="flex items-center justify-between">
                <span className="text-xl font-black italic text-zinc-900 dark:text-white">{stat.val}</span>
        ...
        {/* 3. MOBILE SEARCH BAR (ONLY MOBILE) */}
          ...
          <div className="divide-y divide-zinc-100 dark:divide-white/5">
            {filteredUploads.length > 0 ? (
              filteredUploads.map((file, i) => (
                <div key={i} className="group flex flex-col md:flex-row md:items-center px-5 py-5 md:px-8 md:py-4 hover:bg-zinc-50 dark:hover:bg-blue-600/5 transition-all cursor-pointer active:scale-[0.98] md:active:scale-100">
                  ...
                  <div className="flex items-center gap-4 flex-grow">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="md:hidden text-[8px] font-mono text-blue-600 dark:text-blue-500 font-bold tracking-tighter">{file.id}</span>
                      ...
                      <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                        {file.dept} <span className="w-1 h-1 rounded-full bg-zinc-300 dark:bg-zinc-700"></span> SECURED_NODE
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-4 mt-4 md:mt-0 pt-4 md:pt-0 border-t border-zinc-100 dark:border-white/5 md:border-t-0">
                    <div className="hidden md:block w-24 text-[10px] font-mono text-zinc-400">{file.id}</div>
                    ...
                    <div className="flex items-center gap-4 md:w-56 justify-end">
                      <span className="text-[10px] font-mono font-bold text-zinc-400">{file.size}</span>
                      <span className="hidden md:block text-[10px] font-black italic text-zinc-900 dark:text-white w-24 text-right">{file.date}</span>
                      <div className="hidden md:flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                         <button className="p-1.5 hover:bg-blue-600 hover:text-white rounded-lg transition-all"><ArrowUpRight size={14}/></button>
                         <button className="p-1.5 hover:bg-zinc-200 dark:hover:bg-white/10 rounded-lg transition-all"><MoreVertical size={14}/></button>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="py-24 px-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
                ...
                <div className="relative mb-8">
                  <div className="w-24 h-24 rounded-[2.5rem] bg-zinc-50 dark:bg-white/5 border border-dashed border-zinc-300 dark:border-white/10 flex items-center justify-center relative">
                    <Database size={32} className="text-zinc-200 dark:text-white/10" />
                    <div className="absolute inset-0 border-2 border-blue-500/20 rounded-[2.5rem] animate-[ping_3s_linear_infinite]"></div>
                  </div>
                  <Lock size={16} className="absolute -bottom-1 -right-1 text-blue-600" />
                </div>
                ...
                <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-[0.2em] max-w-[240px] leading-relaxed">
                  No encrypted objects found in your local node directory. Start an uplink to populate.
                </p>
      ...
      {/* 6. TACTICAL FOOTER (MINIMAL ON MOBILE) */}
      <footer className="mt-auto px-6 py-8 flex flex-col md:flex-row justify-between items-center gap-4 opacity-40">
        <div className="flex items-center gap-6">
          <p className="text-[8px] font-black uppercase tracking-widest text-zinc-500">
            Node_Status: <span className="text-blue-600">{userStats.syncStatus}</span>
          </p>
          <p className="text-[8px] font-black uppercase tracking-widest text-zinc-500">
            Enc: <span className="text-zinc-900 dark:text-white">AES-256</span>
          </p>
        </div>
        <p className="text-[8px] font-mono text-zinc-400 uppercase tracking-tighter">
          SYSTEM_ID: DRIFT_ARC_882 // 2024.GLOBAL
        </p>
      </footer>
