...
export default function Page() {
  ...
  const { data, error, isLoading } = useSWR(
    shouldFetch ? `/api/papers?${query}` : null,
    fetcher,
    {
  ...
  const groupedSubjects = papers.reduce((acc, paper) => {
    const key =
      paper.subjectCode ||
      paper.title ||
      `resource-${paper.pageType}-${paper._id}`;

    if (!acc[key]) {
      acc[key] = {
        id: key,
        title: paper.title || "General Resource",
        code: paper.subjectCode || "GEN",
  ...
  const subjects = Object.values(groupedSubjects).map((subject) => ({
    ...subject,
    files: subject.files.sort((a, b) => b.examYear - a.examYear),
  }));
