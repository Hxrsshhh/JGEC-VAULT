...
export default function DepartmentGrid({ year }) {
  ...
  const departments = [
    { id: "cse", label: "Computer Science", code: "CSE", icon: Cpu, color: "from-emerald-500/20 to-emerald-600/5", border: "group-hover:border-emerald-500/50", accent: "emerald-500", stats: "1.2k Files" },
    { id: "it", label: "Information Tech", code: "IT", icon: Network, color: "from-cyan-500/20 to-cyan-600/5", border: "group-hover:border-cyan-500/50", accent: "cyan-500", stats: "980 Files" },
    { id: "ece", label: "Electronics & Comm", code: "ECE", icon: Radio, color: "from-blue-500/20 to-blue-600/5", border: "group-hover:border-blue-500/50", accent: "blue-500", stats: "850 Files" },
    { id: "ee", label: "Electrical Eng", code: "EE", icon: Zap, color: "from-amber-500/20 to-amber-600/5", border: "group-hover:border-amber-500/50", accent: "amber-500", stats: "920 Files" },
    { id: "me", label: "Mechanical Eng", code: "ME", icon: Settings, color: "from-rose-500/20 to-rose-600/5", border: "group-hover:border-rose-500/50", accent: "rose-500", stats: "1.1k Files" },
    { id: "ce", label: "Civil Engineering", code: "CE", icon: HardHat, color: "from-orange-500/20 to-orange-600/5", border: "group-hover:border-orange-500/50", accent: "orange-500", stats: "740 Files" },
  ];
    ...
    <div className="max-w-7xl mx-auto animate-fade-in px-4 md:px-6 lg:px-8 dark:bg-black/10">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-8 mt-4 gap-6 md:gap-0">
        <div className="flex items-center gap-4 md:gap-8 w-full md:w-auto">
          <button
            onClick={() => router.back()}
            className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-zinc-100 dark:bg-white/5 hover:bg-zinc-200 dark:hover:bg-white/10 flex items-center justify-center transition-all border border-zinc-200 dark:border-white/10 group text-zinc-900 dark:text-white shadow-sm flex-shrink-0"
          >
            <ChevronRight
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="w-6 md:w-8 h-[2px] bg-blue-600 rounded-full animate-pulse"></span>
        ...
        {departments.map((dept) => {
          ...
          return (
            ...
            >
              {/* Glow Effect */}
              <div
                className={`absolute inset-0 bg-gradient-to-br ${dept.color} blur-3xl opacity-0 group-hover:opacity-30 dark:group-hover:opacity-60 transition-opacity duration-700 rounded-[2rem] md:rounded-[2.5rem]`}
              />

              <div
                className={`relative h-full p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border bg-white/80 dark:bg-black/20 backdrop-blur-md border-zinc-200 dark:border-white/10 ${dept.border} transition-all duration-500 flex flex-col items-start overflow-hidden shadow-xl hover:shadow-2xl dark:shadow-none hover:-translate-y-1`}
              >
